...
import { useInsideSidebar, useSidebar } from '@/components/ui/sidebar'
  ...
  'relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        destructive: 'bg-destructive text-destructive-foreground focus:bg-destructive/90',
      },
    },
    defaultVariants: {
      variant: 'default',
...
>(({ className, variant, inset, isActive, onClick, preventDefault, ...props }, ref) => {
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      onClick={(event) => {
        ...
        if (isMobile && !preventDefault && isInsideSidebar) {
          setOpenMobile(false)
        }
      }}
      {...props}
